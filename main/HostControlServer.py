...
class HostControlServer(SocketThreadBase):
    ...
    def __init__(self, threadId):
        ...
        self.waitingKetraCmds = []  # Ketra控制命令，主要是房间模式中的节律灯控制
    ...
    def subscribe(self):
        ...
        pub.subscribe(self.controlKetraLights, "control_ketra_lights")
    ...
    def sendKetraCmds(self):
        Utils.logDebug("->sendKetraCmds")
        while True:
            try:
                ketra_param = self.removeFromWaitingKetraiCmds()
                Utils.logInfo("send cmd to Ketra lights: %s" % str(ketra_param))
                if ketra_param is None:
                    break
                ...
                Utils.logError("sendKetraCmds failed...")
    ...
    def controlKetraLights(self, controls=None):
        Utils.logDebug("->controlKetraLights %s" % (controls, ))
        for ketra_param in controls:
            Utils.logDebug("avtivate mode, params: %s" % str(ketra_param))
            self.waitingKetraCmds.append(ketra_param)
    ...
    def removeFromWaitingKetraiCmds(self):
        ketra_param = None
        HostControlServer.__ketralock.acquire()
        try:
            if len(self.waitingKetraCmds) > 0:
                ketra_param = self.waitingKetraCmds[0]
                del self.waitingKetraCmds[0]
                Utils.logInfo("pop first ketra cmd element %d, %s" % (len(self.waitingKetraCmds), ketra_param))
        finally:
            HostControlServer.__ketralock.release()
            return ketra_param
    ...
    def controlDevice(self, deviceVal):
        ...
        if(devType == DEVTYPENAME_LIGHTAJUST):
            try:
                ...
                if curDevValStr == None:
                    ...
                    # 此处处理渐变时间方式与iCasa不同，家卫士APP与ByDelos上设置渐变时间的接口不同
                    ...
                    if(curDevVal is not None):
                        ...
                        curLightingTime = curValue.get("lightingTime","0")
        ...
        if devType == DEVTYPENAME_CURTAIN:  # delos 记录窗帘控制器的开关状态，APP根据状态显示窗帘图片开合 -- chenjc 20170417
            statusDetail = DBManagerDevice().getDeviceByDevAddrAndType(devAddr, devType)
            ctlState = value.get("state")
            if str(ctlState) in ["1", "2"]:
                valueObj = {}
                if statusDetail:
                    valueObj = statusDetail.get("value", {})
                    valueObj["open"] = value.get("state", "1")
                else:
                    statusDetail = {"time": nowTime, "name": deviceName, "type": devType,
                                    "addr": devAddr}  # 如果是None，将statusDetail初始化为新的字典
                    valueObj["open"] = value.get("state", "1")
        ...
        if devType == DEVTYPENAME_CIRCADIAN_LIGHT:
            n4SerialNo = value.get("n4SerialNo")
            n4Detail = DBManagerDeviceProp().getDeviceByAddrType(n4SerialNo, DEVTYPENAME_KETRA_N4)[1]
            if n4Detail:
                n4IPAddr = n4Detail.get("n4IP", "")
                if not n4IPAddr:
                    n4IPAddr = KetraUtils.discoverN4Device(n4SerialNo)

                buttonName = value.get("buttonName")
                keypadName = value.get("keypadName")
                state = value.get("state", "1")
                KetraUtils.activateButton(n4SerialNo, keypadName, buttonName, state, n4IPAddr)  # Activate Ketra button
            else:
                Utils.logError("N4 not found when controlling CircadianLight...")
            return 0
