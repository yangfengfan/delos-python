...
class HostReportServer(ThreadBase):
    ...
    def handleDevStatus(self, devicesValue):
        for deviceVal in devicesValue:
            ...
            if devTypeName == DEVTYPENAME_CURTAIN:  # 窗帘只上报state=0, 添加一个 open字段给APP判断窗帘的开合状态，1开2关
                devAddr = deviceVal.get("addr", None)
                if devAddr:
                    ...
                    if valueInCome:
                        valueInCome["open"] = valueInDb.get("open", "1")
                        deviceVal["value"] = valueInCome
    ...
    def makeAlarmString(self, alarming, addr, devName, almTypeName, produceTime, statusValue=None):
        ...
        if roomname != "":
            msg = '主机：'+ hostName + ',' + roomname + ',' + devName + ','
        else:
            msg = '主机：' + hostName + ',' + devName + ','
    ...
    def processAlmLinkActions(self, newDevAlmRec):
        ...
        for actDevice in actDevices:
            ...
            if devtype == DEVTYPENAME_SCR_OPERATOR:  # 发送
            ...
            # dismiss = tmpDevice.get('dismiss', False)  # delos内设备可以不绑定到房间
            # if dismiss is True:
            #     continue
            ...
            for key in key_list:
                ...
                if devtype in [DEVTYPENAME_LIGHT1, DEVTYPENAME_LIGHT2, DEVTYPENAME_LIGHT3,
                               DEVTYPENAME_LIGHT4, DEVTYPENAME_LIGHTAJUST]:
