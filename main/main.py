...
TID_KAITERRA_CONNECTOR = 111
TID_ROKID_SSDP_SERVER = 112
TID_ROKID_TCP_SERVER = 113
TID_LIGHTADJUST_PANNEL_MONITOR = 114
...
def restartThread(tid):
    ...
    if tid == 0 or tid == TID_KAITERRA_CONNECTOR:
        kaiterraConnector = KaiterraConnector(TID_KAITERRA_CONNECTOR)
        kaiterraConnector.setDaemon(True)
        kaiterraConnector.start()
        time.sleep(1)
        threadTimeAlivingDict[TID_KAITERRA_CONNECTOR] = 0
        threadIdInstanceDict[TID_KAITERRA_CONNECTOR] = kaiterraConnector
        Utils.logDebug("KaiterraConnector is running")
