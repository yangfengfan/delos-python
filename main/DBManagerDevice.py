...
class DBManagerDevice(object):
    ...
    def getDeviceStatusByAddrList(self, deviceAddrList):
        Utils.logDebug("-> getDeviceStatusByAddrList(),deviceAddrList: %s" % str(deviceAddrList))
    ...
    # 批量更新镭豆的数据，仅用于更新镭豆数据
    def updateLaserStatusInBatch(self, devStatusDict):
        addrList = devStatusDict.keys()
        oldStatusDict = self.getDeviceStatusByAddrList(addrList)

        updateParams = []
        for (addr, oldStatusObj) in oldStatusDict.items():
            oldStatusObj['value'] = devStatusDict[addr]
            oldStatusObj['time'] = int(time.time())
            updateParams.append((json.dumps(oldStatusObj), addr))

        updateSql = 'UPDATE ' + self.tablename + " SET detail = ? WHERE addr = ?"
        try:
            conn = DBUtils.get_conn_rt()
            rtn = DBUtils.executemany(conn, updateSql, updateParams)
            return rtn
        except:
