...
class DeviceHandler(WebHandlerBase):
    ...
    def statusfordelos(self, sparam):
        Utils.logDebug("show device status for Delos Display")
        buf = self.sendCommand(GlobalVars.TYPE_CMD_DEVICES_STATUS_FOR_DELOS, sparam)
        return self.successWithMsg(buf)
    ...
    # delos设置和取消常用设备
    def commondevice(self, param):
        buf = self.sendCommand(GlobalVars.TYPE_CMD_UPDATE_DEVICE_FAVORITE, param)
        return self.successWithMsg(buf)
