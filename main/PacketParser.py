...
DEVTYPEID_CIRCADIAN_LIGHT   = 0x17 # 节律灯
...
DEVTYPEID_TABLE_WATER_FILTER = 0x24  # delos台上净水器
DEVTYPEID_AIR_FILTER        = 0x25  # delos空气净化器
DEVTYPEID_FLOOR_WATER_FILTER = 0x26  # delos台下净水器
...
DEVTYPENAME_TABLE_WATER_FILTER = "TableWaterFilter"  # delos台上净水器
DEVTYPENAME_AIR_FILTER        = "AirFilter"   # delos空气净化器
DEVTYPENAME_FLOOR_WATER_FILTER = "FloorWaterFilter"  # delos台下净水器
...
DEVTYPENAME_CIRCADIAN_LIGHT   = "CircadianLight"  # 节律灯
DEVTYPENAME_KETRA_N4          = "N4"  # Ketra N4(By Delos)
DEVTYPENAME_LASER_EGG         = "LaserEgg"  # 镭豆(By Delos)
DEVTYPENAME_SCR_OPERATOR      = "ScrOperator"  # 园区展厅屏幕控制器
...
class PacketParser:  
    ...
    def getDeviceTypeIdByName(deviceTypeName):
        dictDeviceTypeToId = {
                              ...
                              DEVTYPENAME_CIRCADIAN_LIGHT: DEVTYPEID_CIRCADIAN_LIGHT,
    ...
    def getDeviceTypeNameById(deviceTypeId):
        dictDeviceTypeId2Name = {DEVTYPEID_LIGHTAJUST:DEVTYPENAME_LIGHTAJUST,
                                 ...
                                 DEVTYPEID_CIRCADIAN_LIGHT: DEVTYPENAME_CIRCADIAN_LIGHT,
    ...
    def buildControlReqestPack(self, deviceVal):
        try:
            ...
            if DEVTYPEID_LIGHTAJUST == devTypeId:
                ...
                delayTime = value.get('delay', 0)  # 延迟时间，delos展厅特殊需求，2017-0918，chenjc
                if state == "" or lightCoeff == "" or lightingTime == "":
                    Utils.logError('control light, value invalid, state or coeff is empty:%s' % (deviceVal))
                    return None
                bodyBuffer = struct.pack("=4B10s", int(state), int(lightCoeff), int(lightingTime), int(delayTime), "")
            elif DEVTYPEID_LIGHT1 == devTypeId or DEVTYPEID_LIGHT2 == devTypeId or DEVTYPEID_LIGHT3 == devTypeId or \
                            DEVTYPEID_LIGHT4 == devTypeId:
                # state的值：stateN:0,N表示索引为N的灯打开
                ...
                if devTypeId == DEVTYPEID_LCD_SWITCH:  # LCD开关控制灯的命令
                    ...
                    lightValue = lightValue[0:1]
                    bodyBuffer = struct.pack("=3B11s", int(lightIndex), int(lightValue), int(delayTime), "")
                ...
                bodyBuffer = struct.pack("=2B12s", 0, int(value.get("set", "0")), "")
                ...
                if cmd == 1:  # 开关机命令，data_len数值为1
                    bodyBuffer = struct.pack("=3B11s", cmd, 1, int(data), "")
                else:  # 设定温度命令，温度值最大为35,10倍后大于255，所以data_len数值为2
                    data = float(data)
                    if data < 5:
                        data = 5
                    elif data > 35:
        ...
        try:
            ...
            if(devTypeId == DEVTYPEID_LIGHTAJUST):
                ...
                value = {"co2": co2, "voc": self.F2S(voc/100.0, 2), "humidity": self.F2S(humidity/10.0, 1),
                         ...
                         "setSpeed": setSpeed}
            elif devTypeId == DEVTYPEID_TABLE_WATER_FILTER:  # delos台上净水器
                ...
                if package_index[0] == 2:
                    ...
                    pass
            elif devTypeId == DEVTYPEID_AIR_FILTER:  # delos 空气净化器
                ...
                rawTDS, pureTDS, filter1, filter2, filter3, filter4, lackWater, leakWater, rinse = struct.unpack("=2H7B", buffer[0: 11])
                value = {"rawTDS": rawTDS, "pureTDS": pureTDS, "filterLevel1": filter1, "filterLevel2": filter2,
                         "filterLevel3": filter3, "filterLevel4": filter4, "lackWater": lackWater, "leakWater": leakWater,
                         "rinse": rinse}
