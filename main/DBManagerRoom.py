...
class DBManagerRoom(object):
    ...
    def saveRoomProperty(self, detailObj):
        ...
        if(detailObj is None):
            ...
            return None
        try:
            if not detailObj.has_key("areas"):  # delos设备没有区域概念，为了适配服务端增加容错，默认给一个区域
                detailObj["areas"] = [{"areaId": "1", "name":"新区域"}]
            roomId = detailObj.get(KEY_ROOM_ID, None)
            if roomId is None or roomId == "":  # delos的App在新建房间时候roomId可能给的是空字符串  20170402--chenjc
                # return None
